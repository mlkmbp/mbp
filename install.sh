...
CERT_DIR="${CERT_DIR:-/etc/${APP_NAME}/tls}"
BACKUP_DIR="${BACKUP_DIR:-/var/backups/${APP_NAME}}"
GEO_DIR="/etc/${APP_NAME}/dat"
...
preflight_deps(){
  ...
  if ! command -v crontab >/dev/null 2>&1; then
    warn "未检测到 crontab，准备安装定时任务服务..."
    if command -v apt-get >/dev/null 2>&1; then
      install_dependency cron
      systemctl_req enable --now cron || true
    elif command -v yum >/dev/null 2>&1 || command -v dnf >/dev/null 2>&1; then
      install_dependency cronie
      systemctl_req enable --now crond || true
    elif command -v apk >/dev/null 2>&1; then
      install_dependency dcron
    else
      err "不支持的包管理器，无法自动安装 cron，请手动安装后重试。"
      exit 1
    fi
  fi
...
service_exists(){ [ -f "/etc/systemd/system/${APP_NAME}.service" ]; }
...
download_file(){
  local url="$1" out="$2"
  printf "%b\n" "${BLUE}下载：${url}${RESET}"
  if command -v curl >/dev/null 2>&1; then as_root curl -fL --connect-timeout 15 --retry 3 --retry-delay 1 "$url" -o "$out"
  else as_root wget -t 3 -T 20 -O "$out" "$url"; fi
}
...
append_tls_block_to_yaml(){
  local cert="$1" key="$2" domain_or_ip="$3"
  local sni_val="$domain_or_ip"
  if is_ipv4 "$domain_or_ip"; then
    sni_val=""
    ok "检测到是 IP 证书，自动禁用 sniGuard 以兼容浏览器直连"
  fi
...
db:
  forwarder:
    driver: sqlite
    dsn: "file:${DB_DIR}/forwarder.db?_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=synchronous(NORMAL)&_pragma=page_size(32768)&_pragma=foreign_keys(ON)"
    pool:
      max_open: 20
      max_idle: 10
  ...
  log:
    enable: ${LOG_ENABLE}
    driver: sqlite
    dsn: "file:${DB_DIR}/log.db?_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=synchronous(NORMAL)&_pragma=page_size(32768)&_pragma=foreign_keys(ON)"
    pool:
      max_open: 1
      max_idle: 1
      max_lifetime_sec: 0
...
tls_issue_single(){
  local domain="$1" email="$2" restart_now="${3:-0}" extra_ipv6="${4:-}"
  ...
  if [ "$remain" -gt 0 ]; then
    ...
    if [ -f "$conf" ]; then
      ...
      acme_run --renew -d "$domain" --force || { err "续签失败"; restore_port80_services; [ "$had_service" -eq 1 ] && systemctl_req start "${APP_NAME}" || true; return 1; }
    else
      local ext_args=""
      [ -n "$extra_ipv6" ] && ext_args="-d $extra_ipv6"

      if is_ipv4 "$domain"; then
        ok "首次签发纯 IP 证书 (短效策略)：$domain ${extra_ipv6:+(含 IPv6: $extra_ipv6)}"
        acme_run --issue --standalone --server letsencrypt --certificate-profile shortlived --days 6 --httpport 80 --ecc -d "$domain" $ext_args --force || { err "签发失败！已安全中断。"; restore_port80_services; [ "$had_service" -eq 1 ] && systemctl_req start "${APP_NAME}" || true; return 1; }
      else
        ok "首次签发域名证书（standalone + ECC）：$domain"
        if ! acme_run --issue --standalone --listen-v6 --httpport 80 --ecc -d "$domain"; then
          warn "IPv6/80 失败，回退仅 IPv4"
          acme_run --issue --standalone --httpport 80 --ecc -d "$domain" || { err "签发失败！已安全中断。"; restore_port80_services; [ "$had_service" -eq 1 ] && systemctl_req start "${APP_NAME}" || true; return 1; }
        fi
      fi
    fi
  fi
...
tls_request_single_interactive(){
  ok "==> 申请 tls 证书（HTTP-01；直接回车=自动申请本机IP证书，输入 '-' 跳过）"
  while true; do
    d=""; e=""; d_v6=""
    read -rp "请输入域名或IP（直接回车=本机IP，输入 '-' 跳过）： " d || true
    ...
    if [ "$d" = "-" ]; then
      ...
      warn "正在获取本机公网 IP..."
      d="$(curl -s4 -m 5 https://api.ipify.org || curl -s4 -m 5 https://ifconfig.me || true)"
      [ -z "$d" ] && { err "获取 IP 失败，请手动输入"; continue; }
      ok "当前本机 IP 为: $d"
    fi

    is_valid_domain "$d" || is_ipv4 "$d" || { err "域名或 IP 格式不正确"; continue; }

    if is_ipv4 "$d"; then
      read -rp "是否附加本机的 IPv6 地址打进同一个证书？(没有请直接回车跳过): " d_v6 || true
      [ -n "${d_v6:-}" ] && ok "将同时为 $d 和 $d_v6 签发双栈证书"
    fi
    ...
    if tls_issue_single "$d" "$e" "0" "$d_v6"; then
      ...
      warn "tls 申请失败，继续重试（输入 '-' 跳过）"
    fi
  done
}

tls_renew_single_interactive(){
  ok "==> 续签 tls 证书（单域名或IP，成功必覆盖配置并重启）"
  read -rp "输入要续签的域名（例如 example.com；回车取消）: " domain || true
  [ -z "${domain:-}" ] && { ok "已取消"; return 0; }
  is_valid_domain "${domain:-}" || is_ipv4 "${domain:-}" || { err "域名或IP格式不正确"; return 1; }
...
uninstall(){
  ...
  as_root rm -rf "${CONFIG_FILE}" "${LOG_DIR}" "${DB_DIR}" "${FRONTEND_DIR}" "${CERT_DIR}" "${GEO_DIR}" || true
  ok "卸载完成"
}

make_dirs(){ as_root install -d -m 0755 "$INSTALL_DIR" "$LOG_DIR" "$DB_DIR" "$FRONTEND_DIR" "$(dirname "$CONFIG_FILE")" "$GEO_DIR"; }

download_geo_data() {
  ok "==> 拉取 Geo 规则数据库 (geosite / geoip) ..."
  make_dirs
  download_file "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat" "${GEO_DIR}/geosite.dat"
  download_file "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geoip.dat" "${GEO_DIR}/geoip.dat"
}
...
install_flow(){
  ...
  echo "==> 配置网络监听与管理员账号"
  LISTEN_ADDR="$(prompt_clean '监听地址 (默认 0.0.0.0，回车跳过): ' 0)"
  [ -z "$LISTEN_ADDR" ] && LISTEN_ADDR="0.0.0.0"

  while true; do
    LISTEN_PORT="$(prompt_clean '监听端口 (默认 14259，回车跳过): ' 0)"
    [ -z "$LISTEN_PORT" ] && LISTEN_PORT="14259"
    is_valid_port "$LISTEN_PORT" && break || err "端口格式不合法(1-65535)"
  done

  ADMIN_USER="$(prompt_clean '管理员用户名 (回车将随机生成): ' 0)"
  [ -z "$ADMIN_USER" ] && ADMIN_USER="$(tr -dc 'a-zA-Z0-9' </dev/urandom | head -c 8 || true)"

  ADMIN_PASS="$(prompt_clean '管理员密码 (回车将随机生成): ' 0)"
  [ -z "$ADMIN_PASS" ] && ADMIN_PASS="$(tr -dc 'a-zA-Z0-9' </dev/urandom | head -c 12 || true)"

  check_timezone_on_install
  generate_config_file
  download_geo_data

  echo "==> 申请 tls（HTTP-01，单域名；成功则覆盖 tls 段；安装阶段不重启）"
  tls_request_single_interactive || true

  echo "==> 配置 logging.level"
  configure_logging_interactive
  ...
  ok "安装完成: ${APP_NAME}"
  ok "----------------------------------------"
  ok "访问地址: https://${LISTEN_ADDR}:${LISTEN_PORT}"
  ok "用户名:   ${ADMIN_USER}"
  ok "密  码:   ${ADMIN_PASS}"
  ok "========================================"
}
...
while true; do
  ...
  echo "13.  续签 tls 证书"
  echo "14.  列出 tls 证书状态"
  echo "15. 配置 logging.level"
  echo "16. 配置 pve"
  echo "17. 配置 influxdb2"
  ...
  case "${choice}" in
    ...
    12)
      while true; do
        d=""; e=""
        read -rp "请输入域名或IP（单个；回车返回菜单）： " d || true
        [ -z "${d:-}" ] && break
        is_valid_domain "$d" || is_ipv4 "$d" || { err "域名或 IP 格式不正确"; continue; }
